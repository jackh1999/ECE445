...
// GPIO_BUTTON_6 REPLACED BY LCD_LED
...
void app_main(void)
{
    ...
    gpio_config_t buttons_config = {
        .pin_bit_mask = (1ULL << GPIO_BUTTON_0) | (1ULL << GPIO_BUTTON_1) | (1ULL << GPIO_BUTTON_2) | (1ULL << GPIO_BUTTON_3) | \
                        (1ULL << GPIO_BUTTON_4) | (1ULL << GPIO_BUTTON_5) | (1ULL << GPIO_BUTTON_7) | (1ULL << GPIO_BUTTON_8),
    ...
    while((gpio_get_level(GPIO_BUTTON_0) & gpio_get_level(GPIO_BUTTON_1) & gpio_get_level(GPIO_BUTTON_2) & gpio_get_level(GPIO_BUTTON_3) & \
          gpio_get_level(GPIO_BUTTON_4) & gpio_get_level(GPIO_BUTTON_5) & gpio_get_level(GPIO_BUTTON_7) & \
    ...
    while(true){
        ...
        if(gpio_get_level(GPIO_BUTTON_2) == BUTTON_PRESS){
            if(selected_line == 3){
                ...
                double diameter = 0.0254 * wafer_size;
                ...
                double resistance = correction_k * correction_a * correction_f * calculated_voltage / current * 10;
                double resistivity = resistance * thickness;
                double doping = 1.0 / (100.0 * resistivity * mobility * 1.6E-19);
                ...
                ESP_LOGI("Calculated  resitivity", "%f", resistivity);
                ESP_LOGI("Calculated doping", "%f", doping);

                display_results(resistance, resistivity, doping);
                update_menu(panel_handle);
                vTaskDelay(100);
